...
- **Ticket workspace** (`<root>/<ticket-name>/`) — contains clones of repos scoped to a ticket. Tickets sit **directly in the workspace root**, beside `.ocean`. The `tickets/` folder they used to be grouped in is gone; `ggMultiLegacyTicketFolder` keeps it readable, and `WorkspaceUtils.ticketDir` / `ticketDirs` resolve a name or list the tickets of both places. A ticket is recognized by its `ticket.json` (`WorkspaceUtils.isTicketDir`) — that file, not the parent folder name, is what `detectTicketPath` walks up for. **Hidden folders are never tickets**, even with a `ticket.json`: `isTicketDir` rejects a folder whose own name starts with a dot and every folder directly in `.trash` (`ggMultiTrashFolder`, compared in any case like `tickets`), so the `.github`/`.claude`/`.dart_tool` a DNA instantiates in the root, a repo's `.gg` and a closed ticket in `.trash/<ticket>` or `.trash/<ticket> (2)` are no tickets for `detectTicketPath`, `ticketDirs` and `defaultOceanWorkspacePath` alike. Only the folder's own name and the trash count: a workspace root with a hidden name (`~/.ws/<ticket>`) holds tickets like any other. **Ticket names** are checked in one place: `ticketNameError` (and `isValidTicketName`) accepts exactly one visible folder name — not empty, no path (`/`, `\`, absolute), not hidden, not `tickets` in any case — and has to run before a name is joined to a path, because `path.join(root, name)` drops `root` in front of an absolute name; `normalizeTicketName` removes the one trailing separator a tab completion appends and one leading `tickets/` (any case, either separator) when a valid name is left — `tickets/L1/` → `L1`, while `tickets/a/b` and `tickets` stay invalid. Commands that act on a *named* ticket resolve it with `existingTicketDir`, which returns only a real ticket (never an invalid name, a hidden or a plain folder such as `doc`, or a legacy entry that is no real directory). A new ticket folder is created only by `newTicketDir` (used by `do create ticket` and `do import ticket`): it throws for an invalid name, an existing ticket, or a `<root>/<name>` / legacy place taken by something that is no ticket, accepts an empty folder, and creates non-recursively. `ticketDir` merely computes where a ticket of that name would be. From inside a ticket `defaultOceanWorkspacePath` resolves the `.ocean` of `rootOfTicket`, so a legacy `<root>/tickets/<ticket>` finds `<root>/.ocean`; the walk runs over the absolute, normalized working dir. `WorkspaceUtils.rootOfTicket` answers the reverse question and knows both layouts. A folder named `tickets` is the legacy ticket folder only when it holds no `.ocean`/`.master` — otherwise it is a workspace root itself (`~/work/Tickets/<ticket>` resolves `~/work/Tickets/.ocean`); `rootOfTicket` (and with it `Trash` and `cleanUpTicket`), the legacy fallback of `detectTicketPath` and the `tickets` check of `defaultOceanWorkspacePath` all apply this.
- **Trash** (`<root>/.trash/<ticket-name>/`) — the sibling of `.ocean` that holds what gg removed from a ticket. `Trash` (in `lib/src/backend/trash.dart`) owns it. Closing a ticket moves the **whole ticket folder** there in one piece via `moveTicketToTrash`: repositories as they are, `ticket.json`, `.gg/` and the `<ticket>.code-workspace` file. Nothing is deleted. A *non-empty* target — a ticket of the same name closed earlier — gets a ` (2)`, ` (3)`, … suffix, so trash content is never overwritten. `moveFromTicket` (single entries) and `moveFromOcean` follow the same rule; all of them rename and fall back to copy + delete when trash and ticket live on different volumes. Entries do **not** stay forever: `Trash.expire` drops every entry that has been in the trash longer than `Trash.maxAge` (30 days) and returns what it removed; `do create ticket` calls it best-effort, which is the moment the workspace is touched as a whole. An *entry* is a ticket folder below `.trash` or a single repository below `.trash/.ocean/<org>/`, never the `.ocean` mirror as a whole — it collects repositories of wildly different ages. The arrival time is kept in `.trash/.trashed.json`, **not** read off the file system: a `rename` keeps the mtime of the moved folder, so a ticket last touched a year ago would look a year old the second it is trashed. An entry gg never saw arrive — older trash, or something dropped in by hand — is stamped on the run that discovers it and expires `maxAge` later, so no run ever deletes what it only just learned about. **The trash is never a workspace root**: it holds a `.ocean` of its own (`<root>/.trash/.ocean/<org>/<repo>`), so `defaultOceanWorkspacePath` — and with it `defaultGgMultiWorkspacePath` — and `isInsideExistingWorkspace` check neither `.ocean`/`.master` nor a `tickets` folder on the `.trash` level (and migrate no `.master` there) but walk on to its parent; a command run in `<root>/.trash/…` resolves `<root>/.ocean`.
...
| `packageName` / `packageNames(dir)`             | The primary name vs. **every** name a repo declares. A repo that is a Dart *and* an npm package is one repo under two names (`dna_dart` / `@ggdna/dna-dart`, scoped and bare); a dependency may be written either way, so lookups use `packageNames`. |
