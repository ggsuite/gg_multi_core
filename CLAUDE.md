...
- **Ticket workspace** (`<root>/<ticket-name>/`) — contains clones of repos scoped to a ticket. Tickets sit **directly in the workspace root**, beside `.ocean`. The `tickets/` folder they used to be grouped in is gone; `ggMultiLegacyTicketFolder` keeps it readable, and `WorkspaceUtils.ticketDir` / `ticketDirs` resolve a name or list the tickets of both places. A ticket is recognized by its `ticket.json` (`WorkspaceUtils.isTicketDir`) — that file, not the parent folder name, is what `detectTicketPath` walks up for. **Hidden folders are never tickets**, even with a `ticket.json`: `isTicketDir` rejects a folder whose name — or whose parent's name — starts with a dot, so the `.github`/`.claude`/`.dart_tool` a DNA instantiates in the root, a repo's `.gg` and a closed ticket in `.trash/<ticket>` are no tickets for `detectTicketPath`, `ticketDirs` and `defaultOceanWorkspacePath` alike. Commands that act on a *named* ticket resolve it with `existingTicketDir`, which returns only a real ticket (never a hidden or a plain folder such as `doc` of that name); `ticketDir` stays the place a ticket is created in. `WorkspaceUtils.rootOfTicket` answers the reverse question and knows both layouts.
