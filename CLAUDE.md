...
Note: several modules define their own file-local `ProcessRunner` typedef (different shapes). The package export hides `git_platform.dart`'s so `git_snapshot.dart`'s is the one `package:gg_multi_core/gg_multi_core.dart` exposes.
